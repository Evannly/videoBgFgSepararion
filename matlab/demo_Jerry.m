...
readVideo = true;      % Read video or images
resizeScale = 1;        % Scale to resize image.
chanUse = 3;            % Number of Used channels.
...
if readVideo
    vRaw = read(VideoReader('./videos/bootstrap.avi'), [1 nClip]); % Read video. Size: (60,80,3,nClip)
    [h0, w0, c, nFrame] = size(vRaw);   % Get size
    h = floor(h0 * resizeScale);
    w = floor(w0 * resizeScale);
    chanUse = min(chanUse, c);          % Correct chanUse
    ...
    images = double(reshape(vRawClip, [h * w * chanUse, nClip]));
else
    ...
    images = zeros([h * w * chanUse, nfiles]); % image size (120,160,3)
...
if addNoise
    ...
    for idx = 1:noisyFrameNum
        currentimage = reshape(M(:,noisyFrameIdice(idx)),[h,w,chanUse]);
...
fgs = zeros(h, w, chanUse, q);
bgs = zeros(h, w, chanUse, q);
tic;
...
for t = 1:q
    ...
    fgs(:, :, :, t) = imresize(reshape(xt, [h, w, chanUse]), 1 / resizeScale);
    bgs(:, :, :, t) = imresize(reshape(vt, [h, w, chanUse]), 1 / resizeScale);
...
fgsOF = zeros(h, w, chanUse, q);
bgsOF = zeros(h, w, chanUse, q);
tic;
...
for t = 1:q
    ...
    Ztm1OFReshape = reshape(Ztm1OF, [h, w, chanUse, nSI]);
    ...
    fgsOF(:, :, :, t) = imresize(reshape(xt, [h, w, chanUse]), 1 / resizeScale);
    bgsOF(:, :, :, t) = imresize(reshape(vt, [h, w, chanUse]), 1 / resizeScale);
...
%% Show Results
% Reshale results
MReshape = uint8(reshape(M, [h0, w0, c, q]));
fgsReshape = uint8(reshape(fgs, [h0, w0, c, q]));
bgsReshape = uint8(reshape(bgs, [h0, w0, c, q]));
fgsOFReshape = uint8(reshape(fgsOF, [h0, w0, c, q]));
bgsOFReshape = uint8(reshape(bgsOF, [h0, w0, c, q]));
